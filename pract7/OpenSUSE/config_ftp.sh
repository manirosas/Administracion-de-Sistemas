...
function instalar_configurar() {
    ...
    # -------------------------------------------------------
    # REPOSITORIO HTTP PARA PRÁCTICA 7
    # setup_repo.sh escribe aquí: /srv/ftp/http/Linux/{Apache,Nginx,Tomcat}
    # -------------------------------------------------------
    mkdir -p "$FTP_ROOT/http/Linux/Apache"
    mkdir -p "$FTP_ROOT/http/Linux/Nginx"
    mkdir -p "$FTP_ROOT/http/Linux/Tomcat"
    chown -R root:ftp "$FTP_ROOT/http"
    find "$FTP_ROOT/http" -type d -exec chmod 755 {} \;
    echo "  Repositorio HTTP listo en $FTP_ROOT/http/Linux/"
