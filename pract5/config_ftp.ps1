...
function Instalar-Configurar-FTP {
    ...
    # Esperar a que Windows propague los grupos al subsistema de seguridad (SAM)
    ...
    $denyRule = New-Object System.Security.AccessControl.FileSystemAccessRule(
        "$env:COMPUTERNAME\ftp_users", "Delete", "None", "None", "Deny"
    )
    ...
    foreach ($g in $GRUPOS) {
        ...
        $acl.AddAccessRule($rule)
        $denyRule = New-Object System.Security.AccessControl.FileSystemAccessRule(
            "$env:COMPUTERNAME\$g", "Delete", "None", "None", "Deny"
        )
        $acl.AddAccessRule($denyRule)
        Set-Acl $path $acl
    }
